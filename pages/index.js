...
export default function Home() {
  ...
  async function loadNFTs(){
    const provider = new ethers.providers.JsonRpcProvider('https://polygon-mumbai.infura.io/v3/1a4c8e6d0da1413bb8e6d5b37becd5b7');
        ...
        <div className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 ">
          {
            nfts.map((nft, i) =>(
              <div key = {i} className = "border shadow rounded-xl overflow-hidden">
                <Image src = {nft.image} alt = "NFT Image" width = {250} height = {300}/>
                <div className = "p-4">
                  <p style = {{height: '50px', textAlign: 'center'}}className = "text-2xl text-thelinks">
                    {nft.name}
                  </p>
                  <div style = {{height: '20px', overflow: 'hidden'}}>
                    <p className = "text-thelinks">{nft.desc}</p>
                  </div>
                </div>
